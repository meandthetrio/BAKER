...
void PublishEngineLayerParams(UiScreenCtx& ctx)
{
    ...
    for(uint8_t i = 0; i < kPerformLayerCount; ++i)
    {
        const uint16_t clamped_attack = static_cast<uint16_t>(
            ClampInt(static_cast<int>(app.engine.perform_adsr_loop_attack[i]),
        ...
        const uint16_t clamped_release = static_cast<uint16_t>(
            ClampInt(static_cast<int>(app.engine.perform_adsr_loop_release[i]),
                     static_cast<int>(kPerformAdsrAttackReleaseMinMs),
                     static_cast<int>(kPerformAdsrAttackReleaseMaxMs)));
        const uint8_t clamped_decay = static_cast<uint8_t>(
            ClampInt(static_cast<int>(app.engine.perform_adsr_loop_decay[i]), 1, static_cast<int>(kPerformAdsrDecayMaxMs)));
        const uint8_t clamped_sustain = static_cast<uint8_t>(
            ClampInt(static_cast<int>(app.engine.perform_adsr_loop_sustain[i]), 0, static_cast<int>(kPerformAdsrSustainMax)));
        app.engine.perform_adsr_loop_attack[i] = clamped_attack;
        app.engine.perform_adsr_loop_decay[i] = clamped_decay;
        app.engine.perform_adsr_loop_sustain[i] = clamped_sustain;
        app.engine.perform_adsr_loop_release[i] = clamped_release;
        t.engine_drive_mode[i] = ClampDriveMode(static_cast<int>(app.engine.engine_drive_mode[i]));
        t.perform_keyzone_lo_note[i] = app.engine.perform_keyzone_lo_note[i];
        t.perform_keyzone_hi_note[i] = app.engine.perform_keyzone_hi_note[i];
        ...
        t.engine_loop_crossfade_amount[i] = app.engine.perform_adsr_loop_crossfade[i];
        t.engine_loop_crossfade_shape[i] = app.engine.perform_adsr_loop_crossfade_shape[i];
    }
...
{
    const uint32_t applied_gen = app.shared.sd_applied_gen.load(std::memory_order_relaxed);
    ...
    if(s.pcm != nullptr && s.length > 0)
    {
        std::snprintf(app.engine.engine_sample_path[slot],
                      ...
                      "%s",
                      app.ui.sd.last_loaded_path);
        ExtractBaseName(app.ui.sd.last_loaded_path,

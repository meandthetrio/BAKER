...
- Implemented today: Project save/load uses fixed numbered slot files (`PROJECT01.AKPRJ` ... `PROJECT08.AKPRJ`) and includes per-layer WAV paths, per-layer `SampleEdit`, per-layer ENGINE tune, per-layer KEYZONE note bounds, per-layer ADSR submenu state, per-layer EMPHASIS state, seq/plock flags, BPM, LFO wave, macro state, and mod routes.
...
- Current recall rule: `LoadProject` restores saved layer assignments deterministically into explicit slots (`layer A -> slot 0`, `layer B -> slot 1`) instead of the normal inactive-slot path.
- ENGINE tune rule: `LoadProject` also restores `app.engine_tune_semitones[0/1]` and republishes those values through `Params`, so restored tune is immediately visible in ENGINE and audible in playback.
- KEYZONE rule: `LoadProject` also restores `perform_keyzone_lo_note[0/1]` + `perform_keyzone_hi_note[0/1]` and republishes those values through `Params`, so restored KEYZONE ranges match both the screen and note gating.
- ADSR rule: `LoadProject` also restores `perform_adsr_row[0/1]`, `engine_play_mode[0/1]`, per-layer LOOP A/D/S/R, per-layer LOOP crossfade/shape, and per-layer ADSR graph points; the worker republishes the loop/playback subset through `Params`, skips the normal manual-load crossfade default reset for project-driven layer restores, and `PerformAdsr_OnScreenEnter` preserves the restored saved row instead of overwriting it from `engine_play_mode`.
- EMPHASIS rule: `SaveProject` captures drive amount/mode from `app` and filter cutoff/resonance from `Params::TargetsForUI()`; `LoadProject` restores drive amount/mode into `app` and republishes all per-layer EMPHASIS values through `Params`, so the EMPHASIS screen and audible drive/filter state match after recall.
...
- Included today (per `ProjectManifest`): per-layer WAV paths, per-layer `SampleEdit`, per-layer ENGINE tune, per-layer KEYZONE note bounds, per-layer ADSR submenu state, per-layer EMPHASIS state, seq/plock flags, BPM, LFO wave, macro state/selection, mod routes/selection.
...
- Flow: user presses Button1 to open Settings, adjusts `PROJECT SLOT`, then triggers `SAVE PROJECT`/`LOAD PROJECT`; worker writes/reads the selected slot file (`PROJECTNN.AKPRJ`) via temp-file rename (`PROJECTNN.TMP` -> `PROJECTNN.AKPRJ`); project recall restores each saved layer into its explicit slot (`A->0`, `B->1`) inside the existing worker request and republishes saved per-layer ENGINE tune, KEYZONE bounds, ADSR loop/playback state, and EMPHASIS drive/filter state; `project_status` reports slot-tagged save/load/empty/error state; the temporary `ProjectStatus` screen stays visible until REnc click.
- Apply strategy: Load restores per-layer ENGINE tune, KEYZONE note bounds, ADSR loop/playback state, and EMPHASIS drive/filter state immediately through the existing params publish path, seeds per-layer pending edits, then starts explicit-slot WAV restores in sequence; macro/mod state published immediately; audio applies via existing handoff once each layer is ready.
- Where to look: `ui_requests.h` (SaveProject/LoadProject), `ui_worker.cpp` (`SaveProject`, `LoadProject`, explicit-slot restore path), `project_manifest.h` (fields), `app_state.h` (`project_status`, `project_edit_pending_mask`, `project_action_slot`), `ui_screens.cpp` (Settings + ProjectStatus), `ui_overlay.cpp` (SAVE line for WAV renders, not project).

...
- ADSR rule: `LoadProject` also restores `perform_adsr_row[0/1]`, `engine_play_mode[0/1]`, per-layer LOOP A/D/S/R, per-layer LOOP crossfade/shape, and per-layer ADSR graph points; the worker republishes the loop/playback subset through `Params`, and `PerformAdsr_OnScreenEnter` preserves the restored saved row instead of overwriting it from `engine_play_mode`.

...
- Proof (code)
  ...
  - voice_engine_events.cpp: `VoiceEngine::ProcessEvents` drains via `q.Pop(e)` and increments `events_popped`.
...
- Proof (code)
  ...
  - voice_engine_events.cpp: `ProcessEvents`.
  - voice_engine_voice_lifecycle.cpp: `AllocateVoice_`, `NoteOff_`, `AllNotesOff_`.
...
- Proof (code)
  ...
  - voice_engine_voice_lifecycle.cpp: `AllocateVoice_` prefers idle voice; otherwise steals Oldest Note (smallest `start_id`).
  - voice_engine_events.cpp: `note_start_counter_` increments in `ProcessEvents`.
  - voice_engine_voice_lifecycle.cpp: `FinishStopFade_` returns voices to `Idle`.
- Proof (runtime)
...
- Proof (code)
  ...
  - voice_engine_events.cpp: `ProcessEvents` drains the SPSC queue once per block.
...
- Proof (code)
  ...
  - voice_engine_events.cpp: `ProcessEvents` selects `current_sample_`/`sample_bank_`.
  - voice_engine_render.cpp: `RenderBlock` reads `sample->pcm`.
- Proof (runtime)
...
- Status: DONE
- Proof (code)
  - voice_engine_playback.cpp: `SampleAtLinear` / `SampleAtLinearRegion` compute `a + frac * (b - a)`.
  - voice_engine_render.cpp: `RenderBlock` reads PCM via the linear reader during playback.
...
- Status: DONE
- Proof (code)
  - voice_engine_playback.cpp: `ComputeFadeStep` uses `kFadeInMs` (~3 ms) to set per-voice `fade_in_step`.
  - voice_engine_voice_lifecycle.cpp: `StartStopFade_` applies stop fade; `FinishStopFade_` returns voice to idle.
...
- Status: DONE
- Proof (code)
  - voice_engine_playback.cpp: `ComputeRatio` maps MIDI note + `Sample::root_key` to playback ratio.
  - voice_engine_voice_lifecycle.cpp: `StartVoice_` applies `ratio` to per-voice playback.
  - voice_engine_events.cpp: `ProcessEvents` sets it on NoteOn.
...
- Proof (code)
  ...
  - voice_engine_playback.cpp: `InitEnvelope` (Attack/Decay/Sustain setup), `SetEnvelopeRelease`, and `StepEnvelope`.
  - voice_engine_render.cpp: `StepEnvelope` is used per-sample in `RenderBlock` and multiplies the sample (`s *= env_level`).
  - voice_engine_playback.cpp: ADSR time constants are fixed (`kEnvAttackSec`, `kEnvDecaySec`, `kEnvSustainLevel`, `kEnvReleaseSec`).
- Proof (runtime)
...
- Proof (code)
  ...
  - voice_engine_playback.cpp: `AdvancePos` wraps forward when loop enabled (`pos >= loop_end → pos = loop_start`).
  - voice_engine_render.cpp: `RenderBlock` uses loop points during playback.
  - voice_engine_playback.cpp: `SampleAtLinearRegion` reads the looped region.
- Proof (runtime)
...
- Proof (code)
  ...
  - voice_engine_render.cpp: cutoff → `lpf_g` computation + clamp (20–20k Hz; `lpf_g` 0.001–0.999).
  - voice_engine_render.cpp: per-sample update `lpf_z += lpf_g * (s - lpf_z)` in `RenderBlock`.
...
- Status: DONE
- Proof (code)
  - voice_engine_render.cpp: voice summing in `RenderBlock` with `mix_scale = 0.7f / kMaxVoices`.
  - voice_engine_render.cpp: clip detection after scaling; increments `clip_count_` when mix exceeds ±1.0.
...
- Proof (code)
  ...
  - voice_engine_events.cpp: `ProcessEvents` selects from `sample_bank_` using the event’s `sample_index`.
- Proof (runtime)
...
- Proof (code)
  ...
  - voice_engine_events.cpp: `vel_layer` is read from the event payload.
  - voice_engine_voice_lifecycle.cpp: `vel_brightness` drives the soft vs hard timbre setup; no per-layer sample selection.
- Proof (runtime)
...
- Proof (code)
  ...
  - voice_engine_render.cpp: LFO/ModEnv tick per block; values routed to cutoff/pitch via mod matrix.
...
- Proof (code)
  ...
  - voice_engine_render.cpp: reads active route buffer and applies modulation to filter cutoff/pitch.
- Proof (runtime)
...
- Proof (code)
  ...
  - voice_engine_render.cpp: applies `PLocksState` at block boundary (`lock_gen`), overrides cutoff normalization when enabled.
- Proof (runtime)
...
- Proof (code)
  ...
  - voice_engine_render.cpp: applies macros to cutoff/LFO depth/env amount/route0 amount each block.
...
- Proof (code)
  ...
  - ui_worker.cpp: `SaveProject` / `LoadProject` route through fixed slot filenames `PROJECT01.AKPRJ` ... `PROJECT08.AKPRJ` and keep file I/O on the worker path; `LoadProject` now restores saved layer A/B sample paths into explicit slots 0/1 instead of the generic inactive-slot path and republishes saved per-layer ENGINE tune, per-layer KEYZONE note bounds, ADSR loop/playback state, and EMPHASIS drive/filter state through `Params`.
  - project_manifest.h: serialized fields (per-layer wav path/edit, per-layer ENGINE tune, per-layer KEYZONE bounds, per-layer ADSR submenu state, per-layer EMPHASIS state, seq/plock flags, BPM, LFO wave, macros, mod routes) plus legacy v1/v2/v3/v4/v5 upgrade support for older manifests.
...
- Proof (runtime)
  ...
  - Reboot regression fix: save after loading WAV A into ENGINE layer A and WAV B into ENGINE layer B, set distinct KEYZONE note ranges, distinct ADSR submenu state, and distinct EMPHASIS drive/filter settings per layer, reboot, then load the project; each layer returns with its saved sample, tune, KEYZONE range, ADSR row/loop/graph state, and EMPHASIS settings.
...
- Proof (code)
  ...
  - ui_worker.cpp: `SaveProject`/`LoadProject` serialize `ProjectManifest` to fixed numbered slot files and preserve the temp-file rename safety pattern; `LoadProject` restores each saved layer by sequencing explicit target-slot loads inside the existing worker request and republishes saved per-layer ENGINE tune, KEYZONE note bounds, ADSR loop/playback state, EMPHASIS drive/filter state, PROCESS layer master levels, PROCESS FX order, PROCESS SAT state, and PROCESS EQ state through the existing params path; the sample-load completion path now skips the normal manual-load ADSR crossfade reset when that layer load is part of project recall.
  - project_manifest.h: fields persisted (per-layer wav paths, per-layer edits, per-layer ENGINE tune, per-layer KEYZONE low/high notes, per-layer ADSR row/playback state, per-layer ADSR loop A/D/S/R, per-layer ADSR loop crossfade/shape, per-layer ADSR graph points, per-layer EMPHASIS drive amount/mode/cutoff/resonance, per-layer PROCESS layer master levels, PROCESS FX order, PROCESS SAT on/mode/mix/drive/bump/bit-reso/bit-smpl, PROCESS EQ on/mix/center/tilt/Q, seq/plock flags, BPM, LFO wave, macro state, mod routes).
...
- Proof (runtime)
  ...
  - Change state; use Settings to select slot 2 → Save Project; load each slot from Settings and confirm per-layer WAV path + per-layer edit + per-layer ENGINE tune + per-layer KEYZONE range + per-layer ADSR submenu state + per-layer EMPHASIS state + per-layer PROCESS layer master level + PROCESS FX order + PROCESS SAT state + PROCESS EQ state + macros/mod routes restore deterministically for that slot.
  - Layer assignment regression: boot device, load WAV A into ENGINE layer A and WAV B into ENGINE layer B, set distinct tune values, KEYZONE ranges, ADSR settings including non-default loop crossfade amount/shape per layer, distinct EMPHASIS drive/filter settings, distinct PROCESS layer master levels, a distinct non-default PROCESS FX order, a distinct non-default SAT state in each SAT mode family, and a distinct non-default EQ state, save project, power cycle, load project, and confirm each layer returns with the correct sample, tune, KEYZONE bounds, ADSR submenu state, EMPHASIS state, audible PROCESS layer balance, audible PROCESS lane order, audible SAT state, and audible EQ state.
  - Single-layer regressions: repeat with only layer A populated and only layer B populated; confirm the saved layer, tune, KEYZONE range, ADSR submenu state, and EMPHASIS state restore correctly.

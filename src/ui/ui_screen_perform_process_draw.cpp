                                 ...
                                 uint8_t selected_param)
{
    ...
    // Tape saturation only (bit + bias removed): SAT BUMP TONE. The three faders
    // span the full width.
    ...
    constexpr int kSatFaderCount = 3;
    const char* fader_labels[kSatFaderCount] = {"SAT", "BUMP", "TONE"};
    float       fader_values[kSatFaderCount]
        = {t.sat_drive, t.sat_bump, t.sat_tone};
    int  fader_offsets[kSatFaderCount]  = {0, 0, 0};
    bool circle_handles[kSatFaderCount] = {false, false, false};
    bool hide_rails[kSatFaderCount]     = {false, false, false};
    bool hide_handles[kSatFaderCount]   = {false, false, false};
                                   ...
                                   bool rshift_held)
{
    ...
    if(block_h < 3)
        block_h = 3;
    const int fader_x = kMargin;
    // Send-only delay: no SEND/MIX slot, so the three faders span the full width.
    const int fader_w = kDisplayW - fader_x - kMargin;
    if(fader_w <= 4)
        return;
    ...
    // Delay is send-only — no SEND/MIX mode slot is drawn.
}
